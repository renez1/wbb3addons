...
class UserGuestbookData {
    ...
    public function addComment($id,$text) {
        ...
        $sql = "SELECT u.userID"
            ...
            ."\n  LEFT JOIN wcf".WCF_N."_user_guestbook ugb ON (ugb.fromUserID = u.userID)"
            ."\n WHERE ugb.id = ".$id;
        list($fromUserID) = WCF::getDB()->getFirstRow($sql, MYSQL_NUM);
        if(!empty($fromUserID)) {
        	$user = new UserProfile($fromUserID,null,null,null);
            $fromUserName = $user->username;
        	if($fromUserID != $this->userID && $user->userGuestbook_sendCommentPM) {
            	$lang = ($user->languageID == WCF::getLanguage()->getLanguageID() ? WCF::getLanguage() : new Language($user->languageID));
            	$subject = $lang->get('wcf.user.guestbook.pmComment.subject');
            	$msg = $lang->get('wcf.user.guestbook.pmComment.message', array('$username' => $user->username));
            	PMEditor::create(false, array(array("username" => $user->username , 'userID' => $user->userID)), array(), $subject, $msg, $this->userID, $this->user->username);
            }
        }

        // admin pm notifier...
        if(!empty($fromUserID) && !empty($fromUserName) && (USERGUESTBOOK_PMNOTIFIERUGRP || USERGUESTBOOK_PMNOTIFIERUSER)) {
            $notify = array();
            ...
            foreach($notify as $k => $userID) {
                if(!empty($userID) && $userID > 0) {
                    $user = new UserProfile($userID,null,null,null);
                   	$lang = ($user->languageID == WCF::getLanguage()->getLanguageID() ? WCF::getLanguage() : new Language($user->languageID));
                   	$subject = $lang->get('wcf.user.guestbook.pmCommentAdminNotifier.subject', array('$username' => $this->user->username));
                    $msg = $lang->get('wcf.user.guestbook.pmCommentAdminNotifier.message', array('$username' => '[url='.PAGE_URL.'/index.php?page=UserGuestbook&userID='.$this->userID.SID_ARG_2ND.']'.$this->user->username.'[/url]', '$userID' => $this->userID, '$fromusername' => '[url='.PAGE_URL.'/index.php?page=User&userID='.$fromUserID.SID_ARG_2ND.']'.$fromUserName.'[/url]', '$moderator' => $user->username));
                    PMEditor::create(false, array(array("username" => $user->username , 'userID' => $user->userID)), array(), $subject, $msg, $this->userID, $this->user->username, array('enableBBCodes' => 1, 'showSignature' => 0));
                }
            }
        }
    }
