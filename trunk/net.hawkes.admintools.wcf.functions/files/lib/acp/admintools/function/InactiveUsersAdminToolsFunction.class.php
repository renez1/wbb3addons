...
class InactiveUsersAdminToolsFunction extends AbstractAdminToolsFunction {
	...
	public function execute($data) {
		...
		$this->setReturnMessage('success', WCF::getTPL()->get('wcf.acp.admintools.function.user.inactiveUsers.success', array('countDeleted' => count($this->deletedInactiveUsers), 'countWarned' => count($this->warnedInactiveUsers))));
	...
	protected function warnUsers($generalOptions, $warnOnly = false) {
		...
		$sql = "SELECT user.* FROM wcf".WCF_N."_user user
				...
				LEFT JOIN wcf".WCF_N."_user_to_groups user_to_group ON (user_to_group.userID = user.userID)
