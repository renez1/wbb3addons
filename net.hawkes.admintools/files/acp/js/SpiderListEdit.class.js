...
function SpiderListEdit(data, count) {
	...
	this.saveMarkedStatus = function(data) {
		...
		ajaxRequest.openPost('index.php?page=AdminToolsSpiderAction&packageID='+PACKAGE_ID+SID_ARG_2ND, data);
	}
	...
	this.getEditOptions = function(id) {
		...
		//	if (permissions['canMarkItem']) {
				...
				options[i] = new Object();
				options[i]['function'] = 'spiderListEdit.parentObject.markItem(' + (markedStatus ? 'false' : 'true') + ', '+id+');';
	...
	this.remove = function(id) {
		if (confirm(language['wcf.acp.admintools.delete.sure'])) {
				document.location.href = fixURL('index.php?page=AdminToolsSpiderAction&action=delete&itemID='+id+'&url='+encodeURIComponent(url)+SID_ARG_2ND);
		}
	}
	...
	this.removeAll = function() {
		if (confirm(language['wcf.acp.admintools.delete.sure'])) {
			document.location.href = fixURL('index.php?page=AdminToolsSpiderAction&action=deleteAll&url='+encodeURIComponent(url)+SID_ARG_2ND);
		}
	}	
	...
	this.unmarkAll = function() {
		var ajaxRequest = new AjaxRequest();
		ajaxRequest.openGet('index.php?page=AdminToolsSpiderAction&action=unmarkAll'+SID_ARG_2ND);
